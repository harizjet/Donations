...
class _ListMammal extends State<ListMammal> {
  ...
  Future animalReading(String input) async {
    ...
    for (int row = 0; row < table.rows.length; row++) {
      if (table.rows[row][0].toUpperCase() == input.toUpperCase()) {
        for (int col = 0; col < table.maxCols; col++) {
          if (table.rows[row][col] != null) {
            ListMammal.animals[table.rows[row - 1][col]] =
                table.rows[row][col].toString().replaceAll('|', ',');
            ListMammal.found = true;
          }
        }
        return ListMammal.animals;
      }
      ...
      if (table.rows[row][0].toUpperCase().split(
        ...
        ListMammal.closestinput.add(table.rows[row][0]);
      }
    }
  }
  ...
  Widget buildList(BuildContext context, int index) {
    return GestureDetector(
        onTap: () async {
          ListMammal.found = false;
          ListMammal.closestinput = [];
          var temp =
              await animalReading(_ListMammal.animalListMammal[index]['name']);
          if (ListMammal.found == true)
            Navigator.push(
              context,
              MaterialPageRoute(
          ...
          if (ListMammal.found == false)
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => noOutput(
                    ListMammal.found, ListMammal.closestinput.toString()),
              ),
            );
        },
        child: Container(
...
class Search extends SearchDelegate {
  final List animalListMammal;

  final primary = Color(0xff696b9e);
  final secondary = Color(0xfff29a94);

  Search(this.animalListMammal);
