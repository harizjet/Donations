...
class ListReptile extends StatefulWidget {
  ...
  ListReptile(this.animaltype, this.animaltype2, {Key key}) : super(key: key);
  _ListReptile createState() => _ListReptile();
}

class _ListReptile extends State<ListReptile> {
  final TextStyle dropdownMenuItem =
      TextStyle(color: Colors.black, fontSize: 18);
  static List<Map> animalListReptiles = [];
  static List<Map> duplicates = [];

  final primary = Color(0xff696b9e);
  final secondary = Color(0xfff29a94);

  Widget _body = reptilesLOADING();
  ...
  void _imageLoad() async {
    ...
    for (int x = 0; x < animalListReptiles.length; x++) {
      Image downloadImage =
          new Image.network(_ListReptile.animalListReptiles[x]['logoText'].trim());
      ...
      if (x == animalListReptiles.length - 1) {
        if (mounted) {
          setState(() {
  ...
  Future animalReading(String input) async {
    ...
    for (int row = 0; row < table.rows.length; row++) {
      if (table.rows[row][0].toUpperCase() == input.toUpperCase()) {
        for (int col = 0; col < table.maxCols; col++) {
          if (table.rows[row][col] != null) {
            ListReptile.animals[table.rows[row - 1][col]] =
                table.rows[row][col].toString().replaceAll('|', ',');
            ListReptile.found = true;
          }
        }
        return ListReptile.animals;
      }
      ...
      if (table.rows[row][0].toUpperCase().split(
        ...
        ListReptile.closestinput.add(table.rows[row][0]);
      }
    }
  }
  ...
  Future animalData(String animaltype, String animaltype2) async {
    ...
    // }
    setState(() {
      _ListReptile.duplicates = animalListReptiles;
    });
  ...
  Widget reptilesSIAP(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white70,
    body: animalListReptiles == null
          ? Center(
                        ...
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                              ...
                              prefixIcon: Material(
                                ...
                                onPressed: () {
                                  showSearch(context: context, delegate: Search(animalListReptiles));
                                },
                              ),
                              ),
  ...
  Widget buildList(BuildContext context, int index) {
    return GestureDetector(
        onTap: () async {
          ListReptile.found = false;
          ListReptile.closestinput = [];
          var temp =
              await animalReading(_ListReptile.animalListReptiles[index]['name']);
          if (ListReptile.found == true)
            Navigator.push(
              context,
              MaterialPageRoute(
          ...
          if (ListReptile.found == false)
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => noOutput(
                    ListReptile.found, ListReptile.closestinput.toString()),
              ),
            );
        },
        child: Container(
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  image: DecorationImage(
                      image: NetworkImage(
                          _ListReptile.animalListReptiles[index]['logoText']),
                      fit: BoxFit.fill),
                ),
              ),
              Expanded(
                child: Column(
                  ...
                  children: <Widget>[
                    Text(
                      _ListReptile.animalListReptiles[index]['name'],
                      style: TextStyle(
                        color: primary,
...
class reptilesLOADING extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: Align(
...
class Search extends SearchDelegate {
  final List animalListReptiles;

  final primary = Color(0xff696b9e);
  final secondary = Color(0xfff29a94);

  Search(this.animalListReptiles);
